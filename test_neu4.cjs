...
function convertJSArray(arrContent) {
  ...
  while (i < arrContent.length) {
    ...
    if (inStr) {
      ...
      if (ch === strDelim) {
        // ПРИЗНАКИ КОНЦА СТРОКИ в JSON: за " идет , } ] \n \r или конец
        ...
        // Внутренняя кавычка (часть значения, например в SVG атрибуте)
        out.push('\\"');
        i++;
        continue;
      }
...
console.log('jsonStr[260:290]:');
for (let i = 260; i < 291 && i < jsonStr.length; i++) {
  process.stdout.write(i === 281 ? `[${JSON.stringify(jsonStr[i])}]` : JSON.stringify(jsonStr[i]));
}
console.log();
